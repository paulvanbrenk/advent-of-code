...
function part1(input: string[]) {
    ...
    for (let i = 0; i < times.length; i++) {
        ...
        while (l <= r) {
            ...
            log({ i, l, r, m, d });
            if (d < distances[i]) { l = m + 1; }
            else if (d > distances[i]) { leftEdge = Math.min(m, leftEdge); r = m - 1; }
            else { leftEdge = Math.min(m + 1, leftEdge); break; }
        }
        ...
        while (l <= r) {
            ...
            d = distance(m, times[i]);
            log({ i, l, r, m, d });
            if (d > distances[i]) { rightEdge = Math.max(m, rightEdge); l = m + 1; }
            else if (d < distances[i]) { r = m - 1; }
            else { rightEdge = Math.max(m - 1, rightEdge); break; }
        }
...
function part2(input: string[]) {
    ...
    while (l <= r) {
        ...
        log({ l, r, m, d });
        if (d < target) { l = m + 1; }
        else if (d > target) { leftEdge = Math.min(m, leftEdge); r = m - 1; }
        else { leftEdge = Math.min(m + 1, leftEdge); break; }
    }
    ...
    while (l <= r) {
        ...
        d = distance(m, time);
        log({ l, r, m, d });
        if (d > target) { rightEdge = Math.max(m, rightEdge); l = m + 1; }
        else if (d < target) { r = m - 1; }
        else { rightEdge = Math.max(m - 1, rightEdge); break; }
    }
    log({ leftEdge, rightEdge });
