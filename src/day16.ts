...
function fillGrid(grid: readonly string[], start: coords, direction: coords): Set<string> {
    ...
    function fillGridWorker(grid: readonly string[], start: coords, direction: coords): Set<string> {
        ...
        let cnt = 0;
        while (x >= 0 && y >= 0 && x < grid.length && y < grid[0].length && cnt < (100_000)) {
            visited.add(`${x},${y}`);
            const c = grid[x][y];
...
function part2(input: string[]) {
    ...
    for (let i = 0; i < input.length; i++) {
        sources.push([[i, -1], [0, 1]]);
        sources.push([[i, input.length], [0, -1]]);
    }
    for (let i = 0; i < input[0].length; i++) {
        sources.push([[-1, i], [1, 0]]);
        sources.push([[input.length, i], [-1, 0]]);
    }
